...
ClientModule::ClientModule() {
    ...
    std::cout << "Initialising client module" << std::endl;
}

boost::asio::ip::udp::socket& ClientModule::get_socket() {
    return *socket_ptr_;
}

void ClientModule::start() {
    running_ = true;
    module_thread_ = boost::thread([this] {
        
    
    
    // Move the contents of the main function here
    boost::asio::io_context io_context;
    udp::socket socket(io_context, udp::endpoint(udp::v4(), 0));
    socket_ptr_ = &socket; // Add this line
    
    boost::asio::ip::udp::socket::native_handle_type native_socket_descriptor = socket.native_handle();
    std::cout << "Native socket descriptor in class: " << native_socket_descriptor << std::endl;
...
void ClientModule::stop() {
    running_ = false;
    module_thread_.interrupt();
}

void ClientModule::join() {
