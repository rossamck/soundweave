...
#include "Networking/ClientModule.h"
...
{ // Modify the main function to accept command-line arguments
    ...
    else
    {
        ...
        clientModule.start();
        std::cout << "get socket test " << std::endl;

        boost::this_thread::sleep(boost::posix_time::seconds(2)); // replace this with a callback/flag

        boost::asio::ip::udp::socket &socket = clientModule.get_socket();
        // Get the native socket descriptor from the socket object
        boost::asio::ip::udp::socket::native_handle_type native_socket_descriptor = socket.native_handle();

        std::cout << "Native socket descriptor: " << native_socket_descriptor << std::endl;
